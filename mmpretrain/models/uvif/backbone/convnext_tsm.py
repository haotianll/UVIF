...
from .tsm import Sequential_TSM
...
class ConvNeXtBlock(BaseModule):
    ...
    def forward(self, x, **kwargs):
        ...
        def _inner_forward(x, **kwargs):
            shortcut = x
            x = self.depthwise_conv(x, **kwargs)
        ...
        if self.with_cp and x.requires_grad:
            x = cp.checkpoint(_inner_forward, x, **kwargs)
        else:
            x = _inner_forward(x, **kwargs)
        return x
    ...
    def build_tsm(self):
        ...
        if self.shift_place == 'block':
            def make_block_temporal(stage, num_segments):
                ...
                return Sequential_TSM(*blocks)

            for i, stage in enumerate(self.stages):
                self.stages[i] = make_block_temporal(self.stages[i], num_segment_list[i])
            ...
            def make_block_temporal(stage, num_segments):
                ...
                for i, b in enumerate(blocks):
                    if i % n_round == 0:
                        blocks[i].depthwise_conv = TemporalShiftModule(
                            b.depthwise_conv,
                            num_segments=num_segments,
                            shift_div=self.shift_div,
                            is_share=self.is_share,
